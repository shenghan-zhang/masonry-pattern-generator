...
folder='..\Saved_Pictures_temp_martin\E\';
...
option_corner =0; % Option that says if the normal straight pattern should be computed (0) or if the new option should be used (1)
% option = 'uniform'; 
% Lxy = [0.23, 0.18];
% nxy = [0.25, 0.1667];
option = 'by_row';
Ly_ = [0.14,0.14,0.14,0.18,0.18,0.18,0.18]; 
Lx_ = 0.33*ones(1,length(Ly_));
nx_ = 0.2;
ny_ = 0.;   
...
l_edges=0.03; % if 0, no resmampling 
span=5; % The value of the resampled vertices is averaged using a certain span defined by this parameter.
end
%% Fractalization options
if (~exist('in_dedicated_function','var'))
nb_it=1; % Number of fractalization iterration, if 0, no fractalization. (Caution the number of vertices increases as 2^nb_it)
