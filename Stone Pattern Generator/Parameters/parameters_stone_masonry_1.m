...
%%
% define procedures to inforce
do_random_note_addition = true; 
do_merging = false; 
do_subdivision = false;
do_shaking = true; 
do_add_random_points = false;
do_voronoi = true; 
do_erosion = true;
do_resampling = true; 
...
% % nxr = defined in dedicated function;
% % nyr = defined in dedicated function;
...
option = 'by_area';
pe_ = [1.5, 3]; % pe 
len_erosion = [0.009, 0.007];  % dl aimed erosion on straight lines
 %        name : 'gauss' for gaussian, 'exp' for exponential or 'turb' for turbulent
thres_area = 0.03; 
corr_c0 = [0.003, 0.01];
corr_sigma = [0.15, 0.00];
...
% it = defined in dedicated function;
% pe = defined in dedicated function;
% dx = defined in dedicated function;
% dl = defined in dedicated function;
% r  = defined in dedicated function;
% corr.n_random_field = defined in dedicated function;
% corr.name = defined in dedicated function;
% corr.c0 = defined in dedicated function;
% corr.sigma = defined in dedicated function;
end
%% Resampling options
if (~exist('in_dedicated_function','var'))
...
%% Sieving
if (~exist('in_dedicated_function','var'))
min_length_sieving = 0.005; % Minimum length for the sieving step.
end
