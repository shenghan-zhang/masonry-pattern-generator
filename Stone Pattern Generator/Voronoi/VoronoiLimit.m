...
if nargin==0
    ...
    for ii=3:2:nargin
        ...
        elseif strcmp(varargin{ii},'figure')
            fig=varargin{ii+1};
        elseif strcmp(varargin{ii},'epsilon')
            epsilon=varargin{ii+1};
        end
    end
    if exist('fig','var')==0
        fig='on';
...
%%
[vx,vy]=extendVoronoiGraph(vx,vy); % MODIFICATIN BY M.HOFMANN
V=epsilon*round(V./epsilon);
vx=epsilon*round(vx./epsilon);
vy=epsilon*round(vy./epsilon);
%%
vxyl=[vx(:) vy(:)];
xix=ones(size(vx));
...
%% 
if isempty(mix) || min(mix)<1 || max(mod(mix,1))>0
    disp('Mistake with uneven mix members ignored');
end
mix(mod(mix,1)>0)=[];
%%
